...
this.onmessage = function(e){
  switch(e.data.command){
    ...
    case 'getBuffer':
	  ...
	  break;
	case 'setBitDepth':
		bitDepth = e.data.config.bitDepth;
    case 'clear':
      clear();
      break;
  }
...
function init(config){
	console.log("Initialized worker with sampleRate " + config.sampleRate + " and bitDepth " + config.bitDepth);
...
function exportWAV(type){
	if(recLength == 0) {
		throw(new Error("Empty buffer, was the microphone turned on?"));
	} 
	
	console.log("Interleaving raw audio of length " + recLength + " samples");
	var interleaved = mergeBuffers(recBuffers, recLength);
	console.log("Interleaved to length " + interleaved.length + " samples");
	var dataview = encodeWAV(interleaved);
	console.log("Encoded to " + dataview.byteLength + " bytes");
...
function encodeWAV(samples) {
  ...
  /* file length */
  view.setUint32(4, 32 + (samples.length * bytesPerSample), true);
  /* RIFF type */
  ...
  if(bitDepth == 16) {
	  console.log("Encoding float to 16bit pcm");
	floatTo16BitPCM(view, 44, samples);
  } else if(bitDepth == 32) {
	console.log("Encoding float to 32bit pcm");
